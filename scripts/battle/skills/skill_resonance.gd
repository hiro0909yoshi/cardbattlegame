## 共鳴スキル (Resonance/Affinity Skill)
##
...
static func apply(participant: BattleParticipant, context: Dictionary) -> bool:
	...
	if owned_count > 0:
		...
		if ap_bonus > 0 or hp_bonus > 0:
			print("【共鳴発動】", participant.creature_data.get("name", "?"))
