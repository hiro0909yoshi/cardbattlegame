...
##
## 【発動条件】
## - バトル参加クリーチャーが「加勢」キーワードを持つ
## - 対象クリーチャーの属性が加勢対象に含まれる
## - 使用クリーチャーのコスト分のEPを消費
##
## 【効果】
## - 加勢クリーチャーのAPを加算
...
##
## 【特殊処理】
## - ブラッドプリン (ID: 137): 加勢クリーチャーのMHPを永続吸収
##
## 【実装済みクリーチャー】
...
## 加勢効果を適用
##
## 加勢クリーチャーのAP/HPをバトル参加クリーチャーに加算
## ブラッドプリンの場合は特殊処理（MHP永続吸収）
##
## @param participant バトル参加者
## @param assist_creature_data 加勢クリーチャーのデータ
static func apply_assist_effect(participant: BattleParticipant, assist_creature_data: Dictionary) -> void:
	...
	if creature_ap > 0:
		...
		print("  [加勢] AP+", creature_ap, " → ", participant.current_ap)
	
	# HP加算
	if creature_hp > 0:
		participant.item_bonus_hp += creature_hp
		print("  [加勢] HP+", creature_hp, " → item_bonus_hp:", participant.item_bonus_hp)
	
	# 【ブラッドプリン専用処理】加勢クリーチャーのMHPを永続吸収
	if participant.creature_data.get("id") == 137:
		_apply_blood_purin_effect(participant, assist_creature_data)

## ブラッドプリン特殊効果: 加勢クリーチャーのMHPを永続吸収
##
## ブラッドプリン (ID: 137) が加勢クリーチャーを使用した場合、
...
static func _apply_blood_purin_effect(participant: BattleParticipant, assist_creature_data: Dictionary) -> void:
	...
	if actual_increase > 0:
		...
		print("【ブラッドプリン効果】加勢クリーチャー", assist_creature_data.get("name", "?"), "のMHP", assist_mhp, "を吸収")
