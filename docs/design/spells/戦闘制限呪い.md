...
バトル中のクリーチャーに影響を与える刻印システム。

| 刻印名 | curse_type | 効果 |
|-------|-----------|------|
| 錯乱 | `skill_nullify` | 戦闘スキル無効化 |
| 消沈 | `battle_disable` | 攻撃・アイテム・加勢使用不可 |
| 暗転 | `land_effect_disable` | ランドボーナス無効 |
| 恩寵 | `land_effect_grant` | ランドボーナス付与 |
| メタルフォーム | `metal_form` | 無効化[通常攻撃]、防具使用不可 |
| マジックバリア | `magic_barrier` | 無効化[通常攻撃]、攻撃無効化時に敵へ100EP |
| 崩壊 | `destroy_after_battle` | 次の戦闘で生き残った場合、戦闘後に破壊 |
| 免罪 | `creature_toll_disable` | そのドミニオの通行料が0になる |
...
├── 刻印判定: has_xxx(creature_data)
├── 刻印付与: apply_xxx(creature_data)
...
BattleCurseApplier
└── apply_creature_curses() → 刻印をtemporary_effectsに変換
```
...
1. `land_effect_disable`刻印 → false
2. 無属性タイル → true
3. クリーチャー属性一致 → true
4. `extra_land_elements`に含まれる → true
5. `land_effect_grant`刻印 → true
...
|----|------|------|
| 2055 | ディスエレメント | 暗転付与 |
| 220 | スクリーマー | アルカナアーツ: 敵に暗転 |
| 431 | ハイドビハインド | アルカナアーツ: 自に恩寵 |
| 224 | スプラウトリング | 固有: 火から地形効果 |
| 316 | サーペントフライ | 固有: 水から地形効果 |
| 430 | テラリウス | 固有: 全属性地形効果 |
...
## 崩壊（2032 シニリティ / 114 オトヒメ）

### 効果
- 刻印を持つクリーチャーが戦闘で**生き残った場合**、戦闘後に破壊

### 対象カード

| ID | 名前 | タイプ | 効果 |
|----|------|-------|------|
| 2032 | シニリティ | スペル | 対象ドミニオに崩壊刻印付与 |
| 114 | オトヒメ | クリーチャー | 両者生存時、敵に刻印付与（キーワード: `崩壊`） |

### 処理
- スペル: `SpellCurseBattle.apply_destroy_after_battle()`
- クリーチャースキル: `battle_execution.gd`の`_check_apply_destroy_after_battle_skill()`
- 刻印発動: `battle_execution.gd`の`_check_destroy_after_battle()`
...
### 効果
- 刻印を持つクリーチャーのドミニオの通行料が0になる

### 対象カード

| ID | 名前 | タイプ | 効果 |
|----|------|-------|------|
| 124 | スキュラ | クリーチャー | 両者生存時、敵に刻印付与（キーワード: `慈悲`） |

### 処理
- 刻印付与: `SpellCurseBattle.apply_creature_toll_disable()`
...
| ファイル | 役割 |
|----------|------|
| `spell_curse_battle.gd` | 刻印判定・付与API |
| `spell_phase_handler.gd` | スペル使用時の刻印付与 |
| `battle_curse_applier.gd` | 刻印をバトル効果に変換 |
| `battle_special_effects.gd` | 無効化判定（check_nullify） |
| `battle_execution.gd` | 100EP移動、崩壊、スキル刻印付与 |
| `spell_curse_toll.gd` | 免罪判定 |
...
| 2025/12/01 | 3.1 | 崩壊（シニリティ、オトヒメ）、免罪（スキュラ）追加 |
