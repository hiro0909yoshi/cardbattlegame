...
| ファイル | 処理 |
|----------|------|
| `scripts/spells/spell_curse_battle.gd` | `has_battle_disable()`, `apply_battle_disable()`, `check_and_apply_on_attack_success()` |
| `scripts/game_flow/spell_phase_handler.gd` | 呪い付与（スペル）、全体効果 |
| `scripts/game_flow/item_phase_handler.gd` | アイテム・援護強制パス |
| `scripts/battle/battle_execution.gd` | 攻撃スキップ、攻撃成功時呪い付与 |
| `scripts/game_flow/target_selection_helper.gd` | クリーチャー存在チェック |
