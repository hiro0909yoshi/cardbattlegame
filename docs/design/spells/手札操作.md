...
| 2034 | シャッター | 敵手札のアイテム/スペルを1枚選んで破壊 | `destroy_hand_card()` | ⏳ 未実装（UI必要） |
| 2038 | スクイーズ | 手札1枚を選んで破壊、対象にG150を与える | `destroy_hand_card()` + 魔力付与 | ⏳ 未実装（UI必要） |
| 2093 | ポイズンマインド | デッキ上6枚から1枚破壊、1枚ドロー | `destroy_from_deck()` + `draw_cards()` | ⏳ 未実装（UI必要） |
