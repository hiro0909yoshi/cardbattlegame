...
| 2085 | フラットランド | 密命；レベル2領地×5を1上げる | `change_level_multiple_with_condition()` | ✅ 実装完了・動作確認済み |
...
| ID | 名前 | 効果 | 実装メソッド | 実装状況 |
|----|------|------|-------------|---------|
| 2029 | サドンインパクト | 密命；レベル4領地を1下げる | 条件チェック + `change_level()` | ✅ 実装完了 |
| 2085 | フラットランド | 密命；レベル2領地×5を1上げる | `change_level_multiple_with_condition()` | ✅ 実装完了・動作確認済み |
| 2096 | ホームグラウンド | 密命；属性違い領地×4を合う属性に変化 | `find_mismatched_element_lands()` + `align_lands_to_creature_elements()` | ✅ 実装完了 |
