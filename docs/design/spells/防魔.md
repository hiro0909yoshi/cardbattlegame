...
結界は**スペル/アルカナアーツの対象に選べなくなる**効果。ターゲット選択段階でフィルタリングされる。
...
| 種類 | 対象 | 判定場所 |
|------|------|----------|
| パッシブスキル | クリーチャー | `ability_parsed.keywords` に "結界" |
| クリーチャー刻印 | クリーチャー | `curse.curse_type` が "spell_protection" or "protection_wall" |
| プレイヤー刻印 | セプター | `player.curse.curse_type` が "spell_protection" |
| 世界刻印 | 全体 | `game_stats.world_curse`（未実装） |
...
| ファイル | 役割 |
|----------|------|
| `scripts/spells/spell_protection.gd` | 結界判定ロジック（is_creature_protected, is_player_protected） |
...
| `scripts/spells/spell_curse.gd` | curse_player() でプレイヤー刻印付与 |

### フィルタ適用の仕組み

結界フィルタは以下の2箇所で**自動的に**適用される：
...
これにより、どのルートでターゲットを取得しても結界持ちは自動的に除外される。

----------|------|
| `scripts/spells/spell_protection.gd` | 結界判定ロジック |
...
| ID | 名前 | 対象 | 刻印タイプ | 期間 |
|----|------|------|-----------|------|
| 2071 | ブレス | セプター | spell_protection | 5R |
...
| 430 | テラリウス | 無 | neutral_2.json |
| 434 | ビーストギア | 無 | neutral_2.json |

全クリーチャーの `ability_parsed.keywords` に "結界" を設定済み。
...
| ID | 名前 | 効果 | 備考 |
|----|------|------|------|
| 2048 | アンチエレメント | 刻印付きクリーチャーが結界化(6R) | 世界刻印 |
| 2110 | エンプレスドメイン | 全セプターが結界化(6R) | 世界刻印 |
...
- **結界 ≠ 無効化**: 結界は「選べない」、無効化は「選べるが効果なし」
- **マジックシェルターの堅守**: 戦闘処理での対応が別途必要
- **ignore_protection**: target_infoに設定すると結界を無視（ディスペルマジック等用）
