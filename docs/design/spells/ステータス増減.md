...
| ID | 名前 | コスト | 効果 |
|----|------|--------|------|
| 2035 | シュリンクシジル | 30MP | 呪い"縮小術"付与（秘術: 50EPで敵MHP-10） |
...
**秘術付与の仕組み**:
1. スペル使用時、対象クリーチャーに呪いとして秘術情報を付与
2. 呪い内の`mystic_arts`配列が秘術選択UIに表示される
3. 秘術発動時、`spell_id`で`spell_mystic.json`から効果を取得して実行
4. 移動で呪いが消滅すると付与された秘術も消える
