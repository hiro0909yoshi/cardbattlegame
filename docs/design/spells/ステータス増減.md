...
| ID | 名前 | コスト | 効果 |
|----|------|--------|------|
| 2066 | バイタリティ | 30MP | 呪い"暁光"（AP&HP+20）+ ドロー1枚 |
| 2054 | ディジーズ | 30MP | 呪い"衰月"（AP&HP-20）+ ドロー1枚 |
...
| ID | 名前 | コスト | 効果 |
|----|------|--------|------|
| 2120 | リキッドフォーム | 30MP | 呪い"狂星"（AP&HP=10～70ランダム）+ ドロー1枚 |
| 2060 | ドミナントグロース | 50MP | 呪い"昇華"（レベルアップ/地形変化でMHP+20） |
...
| 303 | エアロダッチェス | 50EP・対象敵クリーチャーに呪い"HP-10" (spell_id: 9011) |

### アルカナアーツ付与スペル

| ID | 名前 | コスト | 効果 |
|----|------|--------|------|
| 2035 | ウィザー | 30MP | 呪い"零落"付与（アルカナアーツ: 50EPで敵MHP-10） |
...
呪いとしてアルカナアーツを付与。アルカナアーツを持たないクリーチャーのみ対象。
...
**アルカナアーツ付与の仕組み**:
1. スペル使用時、対象クリーチャーに呪いとしてアルカナアーツ情報を付与
2. 呪い内の`mystic_arts`配列がアルカナアーツ選択UIに表示される
3. アルカナアーツ発動時、`spell_id`で`spell_mystic.json`から効果を取得して実行
4. 移動で呪いが消滅すると付与されたアルカナアーツも消える
...
| SpellCurseStat | ステータス増減スペルの統合処理（恒久変更/呪い/密命） |
| BattleCurseApplier | バトル時の呪い→temporary_effects変換 |
| LandActionHelper | ドミニオオーダー時のcommand_growth呪いトリガー |
...
| 2025/11/28 | ウィザー（アルカナアーツ付与呪い）実装 |
| 2025/11/28 | リキッドフォーム（ランダムステータス呪い）実装 |
| 2025/11/28 | ドミナントグロース（昇華呪い）実装 |
