...
クリーチャーをマップ上で移動させるスペル・アルカナアーツのシステム。移動時は刻印が消滅する。
...
| 2006 | イモビライズ | 全ドミニオに枷刻印付与 | `creature_curse` |
| 2045 | エアリアル | 対象ドミニオに天駆刻印付与 | `creature_curse` |
...
| ID | クリーチャー | 効果 | 効果タイプ |
|----|-------------|------|-----------|
| 10 | クリーピングフレイム | 30EP・1マス移動（堅守） | `move_self` |
| 21 | デスリーチ | 80EP・HP減少+ダウンした敵を破壊し移動 | `destroy_and_move` |
| 218 | ジャングラバー | 30EP・敵に枷刻印付与 | `creature_curse` |
| 28 | バードメイデン | 50EP・自クリーチャーに天駆刻印付与 | `creature_curse` |
| 322 | ゲイルスタリオン | 60EP・自クリーチャーを1~2マス移動 | `move_steps` |
...
```json
{
  "ability_parsed": {
	"keywords": ["奮闘", "アルカナアーツ"],
	"mystic_art": { "name": "天駆付与", "cost": 50, "spell_id": 9014 }
  }
}
```
...
| `scripts/game_flow/movement_helper.gd` | 移動先候補取得、刻印チェック |
...
| `data/spell_mystic.json` | 9013: 枷刻印付与、9014: 天駆刻印付与 |
...
- **堅守の移動**: 敵クリーチャーがいるタイルには枷
- **アルカナアーツの2段階選択**: ゲイルスタリオンは移動対象→移動先の順で選択
