...
クリーチャーへのダメージ・回復処理を行うスペル・アルカナアーツシステム。
...
|-------------|------|--------|----------|
| エルフアーチャー (304) | 対象敵HP-10 | 20EP | 9004 |
| ハードロックドラゴン (231) | 対象敵火/風HP-20 | 60EP | 9005 |
| ライトニングドラゴン (344) | 対象敵水/地HP-20 | 60EP | 9006 |
| アーマードラゴン (404) | 対象敵無HP-30 | 80EP | 9007 |
| ニルーバーナ (428) | 刻印付き全HP-20 | 100EP | 9008 |
| ヒーラー (233) | 対象自HP30回復 | 10EP | 9009 |
...
アルカナアーツはspell_mystic.jsonの`spell_id`を参照して効果を実行。
...
2. `SpellDamage.apply_plague_damage()` で刻印チェック＋ダメージ適用
3. 通知表示（「【衰弱】○○に△△ダメージ！」）
...
**実装方針**:
1. SpellCurseに`hp_effect_nullify`刻印タイプ追加
2. BattleCurseApplierで刻印検出時にフラグ設定
...
SpellDamageは`apply_effect()`メソッドで効果タイプを振り分け：
...
| `scripts/spells/spell_mystic_arts.gd` | アルカナアーツ実行（呼び出しのみ） |
| `data/spell_1.json`, `data/spell_2.json` | スペル定義 |
| `data/spell_mystic.json` | アルカナアーツ用スペル効果（ID: 9001〜） |
