...
| 2084 | パードン | 通行料を支払わない（0G） | 敵プレイヤー | 2T |
...
2. `SpellCurse.curse_player()` で刻印情報を保存（`caster_id` 含む）
3. 刻印情報には duration で有効期間管理
...
toll_info = spell_curse_toll.calculate_final_toll(
	tile_index,          // クリーチャーの刻印チェック
	payer_id,            // 支払い側の刻印チェック
	receiver_id,         // 受取側の刻印チェック
	base_toll
)
  ↓
...
| **toll_disable** | パードン | 支払い = 0G | `calculate_final_toll()` |
...
| ファイル | 役割 |
|---------|------|
| `scripts/spells/spell_curse_toll.gd` | 刻印判定・計算（`calculate_final_toll()` で統一） |
...
| `scripts/spells/spell_curse.gd` | 刻印情報保存（player.curse） |
...
2. **刻印付与**: `SpellCurse.curse_player()` で spell_curse_toll の `caster_id` に「使用者ID」を保存
3. **支払い時**: `calculate_final_toll()` で `caster_id` から副収入受取者を判定
4. **支払い実行**: main_toll をプレイヤーに、bonus_toll を副収入受取者に支払い

### peace の複数処理

敵ドミニオへの移動・戦闘・支払いの各フェーズで peace 刻印をチェック：
- **移動時**: 候補から除外（枷）
