...
**現状**: 完全に未実装。BattleParticipantに既存の効果システム統合ポイントあり（`docs/design/effect_system.md`参照）。
...
| `toll_multiplier` | 通行料倍率 | 通行料×倍率 | `multiplier: 1.5` |
| `peace` | 平和 | 侵略不可+通行料0 | - |
...
| `toll_share` | 通行料共有 | `ratio: 0.5` | MovementController |
...
### Phase 1: 基盤構築
**目標**: SpellCurseクラス作成と基本メソッド実装

- [ ] `spell_curse.gd` クラス作成
- [ ] GameFlowManager統合（setup, update_all_curses）
- [ ] 基本メソッド実装
  - [ ] `curse_creature()` - クリーチャー呪い付与
  - [ ] `curse_player()` - プレイヤー呪い付与
  - [ ] `curse_world()` - 世界呪付与
  - [ ] `get_creature_curse()` - 取得
  - [ ] `get_player_curse()` - 取得
  - [ ] `get_world_curse()` - 取得
  - [ ] `remove_curse_from_creature()` - 削除
  - [ ] `update_all_curses()` - ターン経過処理
- [x] テスト用実装
  - [x] 2100: ホーリーワード6（ダイス固定、最もシンプル）
  - [x] 2066: バイタリティ（能力値+20）

### Phase 2: クリーチャー呪いの統合
**目標**: バトルシステムと移動システムの統合

- [ ] battle_preparation.gd統合（呪い適用）
- [ ] BoardSystem3D統合（移動時削除）
- [ ] バイタリティのバトル適用テスト
- [ ] 2054: ディジーズ（能力値-20）実装
...
| 2025/11/12 | 1.5 | **実装後の修正** - PlayerDataクラスとCreatureManager辞書の違いを明記、正しいアクセス方法を追記 |

---

**作成日**: 2025年11月12日  
**最終更新**: v1.5（実装後の修正 - 正しいアクセス方法を明記）
