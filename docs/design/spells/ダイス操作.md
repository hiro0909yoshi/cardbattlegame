...
| 範囲+EP | 2051 | ジャーニー | 20MP | 2-4範囲+50G/2R | R |
...
スペル使用 → SpellDice.apply_*_effect()
  ↓
SpellCurse.curse_player() - 刻印付与
  ↓
ダイスロール → SpellDice.get_modified_dice_value()
  ↓
EP付与判定 → SpellDice.process_magic_grant()
  ↓
...
### 仕組み
- **付与時**: `duration=1` で刻印を設定
- **ダイスロール時**: 刻印が適用（まだ消えない）
- **ターン終了時**: `duration` が `1 → 0` に減少
- **duration=0**: 刻印が削除

### 重要ポイント
**ターン終了したプレイヤーのみ**刻印を更新（全プレイヤーではない）
...
2. プレイヤー1（敵）を選択
3. 刻印付与: dice_fixed, value=6, duration=1
4. プレイヤー0のターン終了（duration変化なし）
5. プレイヤー1のターン開始 → ダイスロール
6. SpellDice適用 → 6固定
7. プレイヤー1が6マス移動
8. プレイヤー1のターン終了 → duration減少 → 刻印消滅
```

### ジャーニー（巡礼）
```
1. ジャーニー使用 → 刻印付与（duration=2）
2. 【1回目】ダイスロール → 2-4範囲 → EP+50 → 移動
3. ターン終了 → duration=2→1
4. 【2回目】ダイスロール → 2-4範囲 → EP+50 → 移動
5. ターン終了 → duration=1→0 → 刻印消滅
合計: EP100獲得
```
...
| 2025/11/12 | ジャーニー実装 - dice_range_magic追加 |
