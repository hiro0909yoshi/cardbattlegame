...
#### 2. ダウン状態クリア
- プレイヤーの**全ドミニオ**のダウン状態を解除
- 次のターンでドミニオオーダー（レベルアップ/移動/交換）が再び可能に
...
| `description` | | 説明文 |
| `tile_count` | ✓ | タイル総数 |
| `loop_size` | | メインループのタイル数（省略時はtile_count） |
| `tiles` | ✓ | タイル配置データ |
| `connections` | | 分岐タイルの接続情報 |
| `lap_bonus_preset` | | 周回ボーナスプリセット名（デフォルト: "standard"） |
| `checkpoint_preset` | | チェックポイントプリセット名（デフォルト: "standard"） |
...
- [x] 分岐・方向選択システム
- [x] 歩行逆転呪い
- [x] プリセットベース設定システム（game_constants.gd）
