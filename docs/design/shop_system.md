...
| アイテム購入 | ジェム | 回復薬等のアイテムを購入 |
| ジェム購入 | 現金（将来） | ジェムパッケージを購入 |
...
| name | String | 表示名 |
| stone_amount | int | 基本ジェム数 |
| bonus_amount | int | ボーナスジェム数（任意） |
| price | int | 価格（円） |
...
| ファイル | 役割 |
|---------|------|
| `data/stone_packages.json` | ジェムパッケージ定義 |
...
| `scripts/autoload/debug_settings.gd` | ジェム表示フラグ |
