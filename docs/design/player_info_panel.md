...
画面左上に縦積みで配置された小型プレイヤーパネル。各プレイヤーの基本情報（名前、魔力、総魔力）と呪い状態を表示。パネルをクリックするとステータスダイアログが表示され、詳細情報（保有土地、クリーチャー、手札）が確認できる。
...
| `create_world_curse_label()` | 世界呪いラベルを作成 |
...
| `game_flow_manager_ref` | マップ情報（周回、ターン、世界呪い） |
