...
**バージョン**: 1.0  
**作成日**: 2025年10月26日  
**ステータス**: Phase 3 実装中（35体/38体 完了）  
**最終更新**: 2025年10月30日（スペクター実装完了・動作確認済み）

---

## 📊 実装状況サマリー（2025年10月29日）

### 実装完了: 35体（ランダムステータスシステム実装完了）

| カテゴリ | 体数 | クリーチャー |
|----|------|------------|
| **Phase 3-A** | 11 | アイスウォール、トルネード、ファイアードレイク、ブランチアーミー、マッドマン、ガルーダ、アンダイン、アンフィビアン、カクタスウォール、アームドパラディン、サンダースポーン |
| **Phase 3-B** | 6 | ガーゴイル、ネッシー、バーンタイタン、ハイプワーカー、リビングクローブ、タイガーヴェタ |
| **破壊数系** | 3 | バルキリー、ダスクドウェラー、ソウルコレクター |
| **周回・ラウンド系** | 3 | キメラ、ラーバキン、モスタイタン |
| **手札系** | 1 | リリス |
| **Phase 3-C** | 2 | ローンビースト、ジェネラルカン |
| **バトル後変化** | 2 | ロックタイタン、バイロマンサー |
| **アイテム関連** | 2 | リビングアーマー、ブルガサリ |
| **土地イベント** | 2 | アースズピリット、デュータイタン |
| **援護連携** | 1 | ブラッドプリン |
| **ダイス条件** | 1 | ドゥームディボラー |
| **ランダムステータス** | 1 | スペクター |
...
- **コアトリクエ(214)**: CardSystemが全プレイヤー共通デッキのため実装不可
- **リーンタイタン(439)**: バトル後MHP減少（現状実装不可）
- **ギガンテリウム(206)**: マーク付き領地システム未実装のため実装不可
...
- **実装済み**: 33体
  ...
  - 手札系: 1体（リリス）
  - Phase 3-C: 2体（ローンビースト、ジェネラルカン）
  - バトル後変化: 2体（ロックタイタン、バイロマンサー）
  - アイテム関連: 2体（リビングアーマー、ブルガサリ）
  - 土地イベント: 2体（アースズピリット、デュータイタン）
  - **援護連携: 1体（ブラッドプリン）** ← NEW!
- **未実装**: 3体（ドゥームデボラー、スペクター、オーガロード）
- **実装不可**: 3体（コアトリクエ、リーンタイタン、ギガンテリウム）
...
|----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------|--|----|------|------------|---------|-------|----------|-----|----|------|------------|---------|-------|----------|-----|----|------|------------|---------|-------|----------|-----|
| 7 | キメラ | 周回ごとにST+10 | 周回完了 | base_up_ap += 10 | ✅実装済み |
| 23 | ドゥームデボラー | ダイス3以下でST&MHP+10 | 条件満たした移動時 | base_up_ap/hp += 10 | 🚧未実装 |
| 34 | バイロマンサー | 敵攻撃成功後ST=20, MHP-30 | バトル終了時 | base_ap = 20, base_up_hp -= 30 | ✅実装済み |
| 35 | バルキリー | 敵破壊時ST+10 | バトル勝利時 | base_up_ap += 10 | ✅実装済み |
| 137 | ブラッドプリン | 戦闘前MHP+援護クリーチャーHP | 援護発動時 | base_up_hp += support_hp | ✅実装済み |
| 200 | アースズピリット | レベルアップ/地形変化時MHP+10 | 領地変化時 | base_up_hp += 10 | ✅実装済み |
| 227 | ダスクドウェラー | 敵破壊時ST&MHP+10 | バトル勝利時 | base_up_ap/hp += 10 | ✅実装済み |
| 240 | モスタイタン | 周回ごとにMHP+10（80で30にリセット） | 周回完了 | base_up_hp += 10 (check reset) | ✅実装済み |
| 328 | デュータイタン | レベルアップ/地形変化時MHP-10 | 領地変化時 | base_up_hp -= 10 | ✅実装済み |
| 446 | ロックタイタン | バトル後ST-10 | バトル終了時 | base_up_ap -= 10 | ✅実装済み |
| 439 | リーンタイタン | バトル後MHP-10 | バトル終了時 | base_up_hp -= 10 | ❌実装不可 |
...
|----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------|--|----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------|--|----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------|---|----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------||
| 7 | キメラ | per_lap_permanent_bonus | map_lap_count | 🟢永続 | ✅実装済み |
| 240 | モスタイタン | per_lap_permanent_bonus | map_lap_count + reset | 🟢永続 | ✅実装済み |
| 47 | ラーバキン | round_number_bonus | なし | 🔵一時 | ✅実装済み |
...
| ID | 名前 | effect_type | 実装難易度 | 永続性 | ステータス |
|----|------|------------|---------|-------|----------|
| 30 | バーンタイタン | owned_land_count_bonus | ★★☆ | 🔵一時 | ✅実装済み |
| 226 | タイガーヴェタ | adjacent_land_bonus | ★★☆ | 🔵一時 | ✅実装済み |
| 49 | ローンビースト | adjacent_land_bonus + 強打 | ★★★ | 🔵一時 | ✅実装済み |
| 131 | ネッシー | battle_land_level_bonus | ★★☆ | 🔵一時 | ✅実装済み |
| 200 | アースズピリット | on_land_change | ★★★ | 🟢永続 | ✅実装済み |
| 328 | デュータイタン | on_land_change | ★★★ | 🟢永続 | ✅実装済み |
| 206 | ギガンテリウム | marked_land_bonus | ★★★ | 🔵一時 | ❌実装不可 |
...
**実装不可**: ID 206（ギガンテリウム）
- マーク付き領地システムが未実装のため対応不可

---

### 🔴 カテゴリ4: 戦闘イベント（7体）

| ID | 名前 | effect_type | イベント | 永続性 | ステータス |
|----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------|--|----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------|--|----|------|------------|---------|-------|----------|-----|----|------|------------|---------|-------|----------|---|
| 35 | バルキリー | on_enemy_destroy_permanent | on_battle_win | 🟢永続 | ✅実装済み |
| 227 | ダスクドウェラー | on_enemy_destroy_permanent | on_battle_win | 🟢永続 | ✅実装済み |
| 323 | ソウルコレクター | destroy_count_multiplier | 毎バトル | 🔵一時 | ✅実装済み |
| 446 | ロックタイタン | after_battle_permanent_change | on_battle_end | 🟢永続 | ✅実装済み |
| 439 | リーンタイタン | after_battle_permanent_change | on_battle_end | 🟢永続 | ❌実装不可 |
| 34 | バイロマンサー | 特殊処理（ID判定） | on_battle_end | 🟢永続 | ✅実装済み |

**永続バフ**: ID 35（バルキリー）、ID 227（ダスクドウェラー）、ID 34（バイロマンサー）、ID 446（ロックタイタン）
- 戦闘勝利/終了時に `base_up_hp/ap` に加算（または減算・代入）
- 手札に戻るまで永続的に維持
- ✅ **実装完了**（リーンタイタンを除く）

**一時バフ**: ID 323（ソウルコレクター）
- バトルごとに条件チェック（破壊数など）
- バトル終了後は消失
- ✅ **実装完了**

**実装不可**: ID 439（リーンタイタン）
- バトル後MHP減少は現状システムで実装不可
...
| ID | 名前 | effect_type | ステータス |
|----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------|--|----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------|--|
| 438 | リビングアーマー | as_creature_bonus | ✅実装済み |
| 339 | ブルガサリ | on_item_use_bonus | ✅実装済み |
...
| ID | 名前 | effect_type | 永続性 |
|----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------|--|----|------|------------|---------|-------|----------||----|------|------------|---------|-------|----------|--|----|------|------------|---------|-------|----------|---|
| 321 | スペクター | random_stat | 🔵一時 | ✅実装済み |
| 137 | ブラッドプリン | assist_mhp_absorption | 🟢永続 | ✅実装済み |
| 407 | オーガロード | tribe_placement_bonus | 🔵一時 | ✅実装済み |
...
**実装ポイント**:
- ブラッドプリン: ✅ 援護システムとの統合完了（`battle_preparation.gd`）
- スペクター: ✅ 実装完了（`battle_preparation.gd`）
- オーガロード: ✅ 種族システム実装完了（`battle_preparation.gd`）
...
- **実装場所**: MovementController.apply_dice_condition_buffs()
- **トリガー**: ダイスロール後、移動開始前
- **チェック**: ability_parsed.effects に dice_condition_bonus がある場合のみ
- **秘術**: 実装は後回し
- **ステータス**: ✅ 実装完了 (2025-10-30)（現時点では未対応）
...
#### ID 137: ブラッドプリン（援護連携）
```gdscript
# アイテムフェーズで援護クリーチャー使用時
# 1. 通常の援護効果を適用（戦闘中のみ）
participant.item_bonus_hp += assist_creature_hp
participant.current_ap += assist_creature_ap

# 2. ブラッドプリン専用：援護クリーチャーのMHPを永続吸収
var assist_mhp = assist_creature["hp"] + assist_creature.get("base_up_hp", 0)
var current_mhp = creature_data["hp"] + creature_data.get("base_up_hp", 0)
var actual_increase = min(assist_mhp, 100 - current_mhp)
creature_data["base_up_hp"] += actual_increase

# 注意：participant.base_up_hpは更新しない（戦闘後に反映）
```

**重要な仕様**：
- **戦闘中**: 通常の援護効果（`item_bonus_hp`）のみ適用
- **戦闘後**: `item_bonus_hp`消失、`base_up_hp`だけ残る → MHP永続上昇
- **累積**: 援護するたびに加算
...
```
イエティ（HP40）を援護使用:
  戦闘中: item_bonus_hp +40 → current_hp = 60
  戦闘後: base_up_hp +40 → MHP = 60（永続）
```
...
20. **特殊効果** (3体)
    - [x] スペクター (ID: 321) - ランダム ✅
    - [x] ブラッドプリン (ID: 137) - 援護依存 ✅
    - [x] オーガロード (ID: 407) - 種族判定 ✅
...
| 2025/10/28 | 1.3 | ID 1, 318実装済みに変更、ID 446, 439を永続バフに変更、合計24体実装完了 |
| 2025/10/28 | 1.4 | ロックタイタン（ID: 446）実装完了、合計27体実装完了 |
| 2025/10/29 | 1.5 | バイロマンサー（ID: 34）、リビングアーマー（ID: 438）、ブルガサリ（ID: 339）実装完了、合計30体実装完了 |
| 2025/10/29 | 1.6 | アースズピリット（ID: 200）、デュータイタン（ID: 328）実装完了、リーンタイタン（ID: 439）、ギガンテリウム（ID: 206）を実装不可に変更、合計32体実装完了 |
| 2025/10/29 | 1.7 | ブラッドプリン（ID: 137）実装完了、援護クリーチャーのMHP吸収機能追加、合計33体実装完了 |
| 2025/10/29 | 1.8 | ブラッドプリン実装修正完了、援護システムとの統合（通常援護+永続MHP吸収） |
| 2025/10/29 | 1.9 | ブラッドプリン動作確認完了、攻撃側・防御側両方で援護システム正常動作確認 |
| 2025/10/30 | 2.0 | ドゥームディボラー（ID: 23）実装完了、ダイス条件バフシステム追加、合計34体実装完了 |
| 2025/10/30 | 2.1 | スペクター（ID: 321）実装完了、ランダムステータスシステム追加、合計35体実装完了 |

---

**最終更新**: 2025年10月30日（ドゥームディボラー実装完了、残り2体）（v2.0）
