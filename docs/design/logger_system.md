...
| カードドロー | 毎ターン発生する日常動作。バグ原因になりにくい | STEP 2以降 |
...
既存の `push_error()` 約286件のうち、Logger.error() に変換すべきものを以下に分類する。
...
→ **全て Logger.error()。** ロジックバグまたはデータ破損を示す。
...
```gdscript
# 基本ログ
Logger.info(tag: String, message: String)
Logger.warn(tag: String, message: String)
Logger.error(tag: String, message: String)
```
...
```gdscript
# ターン開始
Logger.info("GFM", "ターン開始: P%d, ラウンド%d" % [player_id + 1, round_number])

# フェーズ遷移
Logger.info("SM", "フェーズ遷移: %s -> %s" % [from_name, to_name])

# ダイス結果
Logger.info("Dice", "ダイス: P%d 出目%d (修正後:%d)" % [player_id + 1, raw_dice, modified_dice])

# 移動完了（出発タイルと着地タイルの両方を記録 → 出目との整合性が検証できる）
Logger.info("Move", "移動完了: P%d タイル%d → タイル%d" % [player_id + 1, from_tile, final_tile])

# スペルフェーズ開始（フェーズの開始/完了ペアでフリーズ検知）
Logger.info("Spell", "スペルフェーズ開始: P%d 手札スペル%d枚" % [player_id + 1, spell_count])

# スペル選択開始（スペルカード選択直後、ターゲットUI表示前）
Logger.info("Spell", "選択開始: P%d %s(id:%d)" % [player_id + 1, spell_name, spell_id])

# スペル選択確定（ターゲット選択完了時。対象種別 + ID + タイル位置）
Logger.info("Spell", "選択確定: P%d %s(id:%d) → %s:%s tile:%d" % [
...
Logger.info("Spell", "効果完了: P%d %s(id:%d) result=%s" % [
    player_id + 1, spell_name, spell_id, result])
# result: "success", "fail", "no_effect"

# アーツ選択開始（アーツ選択直後）
Logger.info("Spell", "アーツ選択開始: P%d %s(id:%d)" % [player_id + 1, arts_name, arts_id])

# アーツ選択確定（ターゲット選択完了時）
Logger.info("Spell", "アーツ選択確定: P%d %s(id:%d) → %s:%s tile:%d" % [
    player_id + 1, arts_name, arts_id, target_type, target_id, tile_idx])

# アーツ効果完了
Logger.info("Spell", "アーツ効果完了: P%d %s(id:%d) result=%s" % [
    player_id + 1, arts_name, arts_id, result])

# CPU版も同じ形式（CPUプレフィックスで区別）
Logger.info("Spell", "アーツ選択開始: CPU P%d %s(id:%d)" % [player_id + 1, arts_name, arts_id])
Logger.info("Spell", "アーツ効果完了: CPU P%d %s(id:%d) result=%s" % [
    player_id + 1, arts_name, arts_id, result])

# スペルフェーズ完了（開始とペア）
Logger.info("Spell", "スペルフェーズ完了: P%d 使用%d枚" % [player_id + 1, used_count])

# バトル結果（クリーチャー名 + ID + HP変化 — 即死バグ・ダメージ異常の検出）
Logger.info("Battle", "バトル結果: %s(id:%d) vs %s(id:%d) → %s勝利 (HP:%d→%d, タイル%d)" % [
    atk_name, atk_id, def_name, def_id, winner, loser_hp_before, loser_hp_after, tile_idx])

# バトルUI開始（seq + phase 付き。開始・終了のペアで追跡）
# _battle_seq はBattleScreenManagerのメンバ変数（var _battle_seq: int = 0）
Logger.info("BattleUI", "開始[seq:%d]: %s(id:%d) vs %s(id:%d) タイル%d phase:%s" % [
    _battle_seq, atk_name, atk_id, def_name, def_id, tile_idx, phase_name])

# バトルUI終了（seqで開始とペア対応。タイムスタンプ差が大きければTween詰まりの疑い）
Logger.info("BattleUI", "終了[seq:%d]: %s" % [_battle_seq, result_name])

# バトルUI異常（異常検知時のみ記録 — 正常時はstateを出さない）
Logger.error("BattleUI", "BattleScreen取得失敗[seq:%d]: pool empty" % _battle_seq)
Logger.warn("BattleUI", "重複検出[seq:%d]: _battle_screen=%s, _is_battle_active=%s" % [
    _battle_seq, str(_battle_screen != null), str(_is_battle_active)])
Logger.warn("BattleUI", "演出スキップ[seq:%d]: _battle_screen が null (show_attack)" % _battle_seq)

# 召喚成功（クリーチャー名 + ID + コスト）
Logger.info("Summon", "召喚: P%d %s(id:%d) → タイル%d (%s) コスト:%dEP" % [
    player_id + 1, creature_name, creature_id, tile_idx, element, cost])

# 召喚失敗（理由を含める — なぜ出せなかったかが重要）
Logger.warn("Summon", "召喚失敗: P%d %s(id:%d) → %s (タイル%d)" % [
    player_id + 1, creature_name, creature_id, reason, tile_idx])
# reason例: "土地条件未達", "配置制限", "EP不足", "堅守は空き地のみ"

# ドミニオコマンド実行（コマンド種類 + タイル + クリーチャー）
Logger.info("Dominio", "コマンド: P%d %s タイル%d %s(id:%d)" % [
    player_id + 1, action_type, tile_idx, creature_name, creature_id])
# action_type: "level_up", "move_creature", "swap_creature", "terrain_change"

# 移動侵略（from → to。バトル結果は Battle タグで別途記録される）
Logger.info("Dominio", "移動侵略: P%d タイル%d → タイル%d %s(id:%d)" % [
    player_id + 1, from_tile, to_tile, creature_name, creature_id])

# ドミニオコマンド完了（開始とペア。成否を含める）
Logger.info("Dominio", "コマンド完了: %s タイル%d result=%s" % [action_type, tile_idx, "success" if success else "failed"])

# ドミニオ異常（ステート不整合・無効参照）
Logger.warn("Dominio", "ステート不整合: 期待=%s, 実際=%s" % [expected_state, actual_state])
Logger.error("Dominio", "移動元タイルが無効: move_source_tile=%d" % move_source_tile)

# ===== ゲーム開始/終了 =====

# ゲーム開始（ログの始まり。セッション全体の区切り）
Logger.info("Game", "ゲーム開始: %d人 ステージ:%s" % [player_count, stage_name])

# ゲーム終了（ログの終わり）
Logger.info("Game", "ゲーム終了: ラウンド%d" % round_number)

# 勝利判定（チーム合算TEPの場合もある）
Logger.info("Game", "勝利: P%d TEP:%d (目標:%d)" % [player_id + 1, tep, target])

# 破産（プレイヤー脱落の重大イベント）
Logger.warn("Game", "破産: P%d EP:%d" % [player_id + 1, ep])

# ===== 通行料 =====

# 通行料支払い（残EPを含めると破産直前の状況が即判明）
Logger.info("Toll", "通行料: P%d → P%d %dEP (タイル%d Lv%d 残EP:%d)" % [
    payer_id + 1, owner_id + 1, amount, tile_idx, level, remaining_ep])

# ===== 周回/チェックポイント =====

# チェックポイント通過（EPボーナス付与）
Logger.info("Lap", "チェックポイント通過: P%d タイル%d EP+%d" % [player_id + 1, tile_idx, bonus])

# 周回完了（ダウン全解除 + HP全回復 + EPボーナス + 永続ステータスボーナスが走る）
Logger.info("Lap", "周回完了: P%d %d周目 ダウン全解除+HP回復" % [player_id + 1, lap_count])

# ===== エラー =====

# エラー（ターンとフェーズを必ず含める — ログ単体で状況がわかる）
Logger.error("Spell", "spell_land: tile_data が null (turn:%d, phase:%s, tile:%d)" % [turn, phase_name, tile_index])
```
...
| `spell_effect_executor.gd` | バリデーション失敗・null参照時 | スペルエラー（push_error→Logger.error置換） |
...
1. STEP 1 では Logger 埋め込み箇所の `print()` のみ `Logger.info()` に置換
