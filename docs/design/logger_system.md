...
GameLogger.info("Spell", "選択開始: P%d %s(id:%d)" % [player_id + 1, spell_name, spell_id])
...
# アーツ選択開始（アーツ選択直後）
GameLogger.info("Spell", "アーツ選択開始: P%d %s(id:%d)" % [player_id + 1, arts_name, arts_id])

# アーツ選択確定（ターゲット選択完了時）
GameLogger.info("Spell", "アーツ選択確定: P%d %s(id:%d) → %s:%s tile:%d" % [
    player_id + 1, arts_name, arts_id, target_type, target_id, tile_idx])

# アーツ効果完了
GameLogger.info("Spell", "アーツ効果完了: P%d %s(id:%d) result=%s" % [
    player_id + 1, arts_name, arts_id, result])

# CPU版も同じ形式（CPUプレフィックスで区別）
GameLogger.info("Spell", "アーツ選択開始: CPU P%d %s(id:%d)" % [player_id + 1, arts_name, arts_id])
GameLogger.info("Spell", "アーツ効果完了: CPU P%d %s(id:%d) result=%s" % [
    player_id + 1, arts_name, arts_id, result])
...
GameLogger.info("Battle", "バトル結果: %s(id:%d) vs %s(id:%d) → %s勝利 (HP:%d→%d, タイル%d)" % [
...
GameLogger.info("Dominio", "コマンド: P%d %s タイル%d %s(id:%d)" % [
