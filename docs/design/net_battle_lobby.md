...
### NetBattleSetup.tscn（準備画面 / 1シーン・ホスト/ゲスト切替）

マッチング成立後の待機・設定画面。`net_battle_mode` メタデータの `is_host` フラグでUI要素の操作可否を切り替え。
...
|--------|--------|--------|
| ブック選択 | 操作可 | 操作可 |
| マップ選択 | 操作可 | disabled + 「ホストが選択中」表示 |
| ルール設定（プリセット） | 操作可 | disabled |
| ルール設定（SpinBox ▲▼） | 操作可 | disabled |
| プレイヤーリスト | 表示のみ | 表示のみ |
| 3Dキャラプレビュー | 表示（各スロット横） | 表示（各スロット横） |
| 下部ボタン | 「対戦開始」（全員Ready後に黄色で有効化） | 「準備完了」（トグル式） |

切り替えは `GameData.set_meta("net_battle_mode")` で渡す：
```gdscript
# ホスト
GameData.set_meta("net_battle_mode", {"is_host": true, "max_players": 4})
# ゲスト
GameData.set_meta("net_battle_mode", {"is_host": false, "max_players": 4})
```
...
| ファイル | 状態 | 備考 |
|---------|------|------|
| `scenes/NetBattleSetup.tscn` | 実装済み | フレンドマッチ準備画面 |
| `scripts/net_battle_setup.gd` | 実装済み | ホスト/ゲスト切替、3Dキャラプレビュー付き |
| `scripts/main_menu.gd` | 修正済み | ネット対戦ボタン → NetBattleSetup遷移 |
| `scenes/NetBattleLobby.tscn` | 未実装 | ランクマッチ/フレンドマッチのタブ切替ロビー |
| `scripts/net_battle_lobby.gd` | 未実装 | |
| `scripts/network/network_service.gd` | 未実装 | 抽象レイヤー（既存 network_manager.gd あり） |
