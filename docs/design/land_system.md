...
| 未所有 | 黒い枠・発光なし |
| 所有中（ダウン状態） | 黒い枠・発光なし |
| 所有中（アクティブ） | プレイヤー色・常時発光（点滅なし） |

**実装詳細**:
- アクティブ中のみ emission_enabled = true（emission_energy_multiplier = 1.0 固定）
- 点滅は廃止（`_process` 常時稼働を無くし、所有タイル分のCPU負荷を削減）
- 到着予測ハイライト（黄色点滅）のみ `_process` で動作
...
| 2026/04/15 | 1.9 | ビジュアル仕様変更: ダウン=発光なし/アクティブ=常時発光（点滅廃止）。所有タイルの `_process` 削減でCPU負荷軽減 |
