...
### TEPへの影響
所持している石の**現在価値**がTEPに加算される

```
TEP = 所持EP + 土地価値 + 石の現在価値合計
```
...
- `scripts/player_system.gd` - 石所持データ（PlayerData.magic_stones）、TEP計算
...
| 2025/12/18 | 魔法石タイル基盤実装（動的価格、売買UI）、TEP計算をPlayerSystemに一元化 |
