# 死亡時効果スキル

**バージョン**: 1.0  
**最終更新**: 2025年11月2日

---

## 📋 目次

1. [概要](#概要)
2. [発動タイミング](#発動タイミング)
3. [効果タイプ](#効果タイプ)
4. [実装場所](#実装場所)
5. [効果詳細](#効果詳細)
6. [実装クリーチャー・アイテム](#実装クリーチャーアイテム)
7. [他スキルとの相互作用](#他スキルとの相互作用)
8. [実装コード](#実装コード)
9. [使用例](#使用例)
10. [設計思想](#設計思想)

---

## 概要

クリーチャーが戦闘で撃破された時に発動する特殊効果。相手にダメージを与えたり、特殊な影響を与える。

**特徴**:
- trigger `"on_death"` で統一
- 撃破された側のアイテム/能力から発動
- 侵略側/防衛側に関係なく動作
- 複数の効果タイプを統合管理

---

## 発動タイミング

### タイミング詳細

```
攻撃実行
  ↓
ダメージ処理
  ↓
【撃破判定】← ここで死亡時効果をチェック
  ↓
死者復活チェック
  ↓
次の攻撃 or 戦闘終了
```

### 発動条件

1. **使用者（装備者）が撃破された**
2. **相手が生存している**（効果によっては不要）
3. trigger が `"on_death"` である

---

## 効果タイプ

### 1. instant_death（道連れ）

相手を即死させる効果。

**特徴**:
- 確率判定あり（デフォルト100%）
- target `"attacker"` で相手を指定
- 即死フラグを立てる

### 2. revenge_mhp_damage（雪辱）

相手のMHP（最大HP）に直接ダメージ。

**特徴**:
- ボーナスHPを無視してMHP範囲を削る
- MHPが0以下になると即死
- 相手が生存している場合のみ発動

---

## 実装場所

### battle_special_effects.gd

**関数**: `check_on_death_effects(defeated: BattleParticipant, opponent: BattleParticipant) -> Dictionary`

**配置理由**:
- 特殊な戦闘効果を扱う専門クラス
- 道連れ、雪辱など複数の死亡時効果を統合管理
- 戦闘フロー（battle_execution.gd）から分離

**呼び出し箇所**:
- `battle_execution.gd` の攻撃処理内
- 撃破判定の直後

---

## 効果詳細

### instant_death（道連れ）

#### JSON形式

```json
{
  "effect_parsed": {
	"effects": [
	  {
		"effect_type": "instant_death",
		"trigger": "on_death",
		"target": "attacker",
		"probability": 100
	  }
	]
  }
}
```

#### パラメータ

| パラメータ | 型 | 必須 | デフォルト | 説明 |
|-----------|-----|------|-----------|------|
| `effect_type` | String | ✅ | - | "instant_death" |
| `trigger` | String | ✅ | - | "on_death" |
| `target` | String | ✅ | - | "attacker"（相手を指定） |
| `probability` | int | ❌ | 100 | 発動確率（%） |

#### 処理フロー

```gdscript
1. 確率判定（randf() * 100.0 <= probability）
2. 成功 → opponent.instant_death_flag = true
3. opponent.base_hp = 0
4. opponent.update_current_hp()
```

#### 実装例

**バーニングハート（ID: 1048）**
```json
{
  "id": 1048,
  "name": "バーニングハート",
  "effect": "AP+|AP&|AP=HP+20；道連れ",
  "effect_parsed": {
	"stat_bonus": {"ap": 20, "hp": 20},
	"effects": [
	  {
		"effect_type": "instant_death",
		"trigger": "on_death",
		"probability": 100,
		"target": "attacker"
	  }
	]
  }
}
```

---

### revenge_mhp_damage（雪辱）

#### JSON形式

```json
{
  "effect_parsed": {
	"effects": [
	  {
		"effect_type": "revenge_mhp_damage",
		"trigger": "on_death",
		"damage": 40
	  }
	]
  }
}
```

#### パラメータ

| パラメータ | 型 | 必須 | デフォルト | 説明 |
|-----------|-----|------|-----------|------|
| `effect_type` | String | ✅ | - | "revenge_mhp_damage" |
| `trigger` | String | ✅ | - | "on_death" |
| `damage` | int | ❌ | 40 | MHPダメージ量 |

#### 処理フロー

```gdscript
1. 相手が生存しているかチェック
2. opponent.take_mhp_damage(damage)
3. base_up_hp → base_hp の順で消費
4. MHPが0以下 → 即死
```

#### MHPダメージの仕組み

通常のダメージ消費順序：
```
1. resonance_bonus_hp（感応）
2. land_bonus_hp（土地ボーナス）
3. temporary_bonus_hp（一時ボーナス）
4. item_bonus_hp（アイテム）
5. spell_bonus_hp（スペル）
6. current_hp（残りHP） ← base_hp の現在値
```

**雪辱の特徴**:
- 1〜5のボーナスを無視
- MHP範囲（base_hp）に直接ダメージ
- **base_up_hp は削られない**（永続的なMHPボーナスのため）
- 高レベル土地やバフで守られた敵を倒す手段

#### 実装例

**ナパームアロー（ID: 1044）**
```json
{
  "id": 1044,
  "name": "ナパームアロー",
  "effect": "AP+|AP&|AP=30；HP+20；雪辱[敵のMHP-40]",
  "effect_parsed": {
	"stat_bonus": {"ap": 30, "hp": 20},
	"effects": [
	  {
		"effect_type": "revenge_mhp_damage",
		"trigger": "on_death",
		"damage": 40
	  }
	]
  }
}
```

**使用例**:
```
敵：MHP=50、土地ボーナス+30 → 現在HP=80
攻撃：AP30 → 土地ボーナス削られる → HP=50
反撃：死亡
雪辱発動 → MHP 50 - 40 = 10 → 敵HP=10（生存）

もしMHP=30なら:
雪辱発動 → MHP 30 - 40 = -10 → 敵死亡
```

---

## 実装クリーチャー・アイテム

### instant_death（道連れ）

#### アイテム
- **バーニングハート（ID: 1048）**: AP+|AP&|AP=HP+20；道連れ[100%]

### revenge_mhp_damage（雪辱）

#### アイテム
- **ナパームアロー（ID: 1044）**: AP+|AP&|AP=30；HP+20；雪辱[MHP-40]

---

## 他スキルとの相互作用

### 道連れ（instant_death）

#### 無効化スキル
- ❌ **無効化では防げない**（死亡後の効果）
- ❌ **反射では防げない**（死亡後の効果）

#### 死者復活
- ✅ **道連れ発動後に死者復活可能**
- 道連れで相手を倒した後、使用者が復活する可能性

#### スクイドマントル
- ❌ **防げない**（スクイドマントルは攻撃時の即死を無効化するが、道連れには無効）

### 雪辱（revenge_mhp_damage）

#### 無効化スキル
- ❌ **無効化では防げない**（MHP範囲への直接ダメージ）
- ❌ **反射では防げない**（死亡後の効果）

#### 再生スキル
- ⚠️ **雪辱後に再生が発動する可能性**
- MHPが削られた後、残りHPまで回復

#### バフ効果
- ✅ **土地ボーナス、アイテムボーナスなどを無視**
- MHP範囲（base_hp + base_up_hp）に直接作用

---

## 実装コード

### battle_special_effects.gd

```gdscript
## 死亡時効果のチェック（道連れ、雪辱など）
func check_on_death_effects(defeated: BattleParticipant, opponent: BattleParticipant) -> Dictionary:
	"""
	撃破された側の死亡時効果をチェックして発動
	
	Args:
		defeated: 撃破されたクリーチャー（死亡した側）
		opponent: 相手クリーチャー（生き残った側）
	
	Returns:
		Dictionary: {
			"death_revenge_activated": bool,  # 道連れが発動したか
			"revenge_mhp_activated": bool     # 雪辱が発動したか
		}
	"""
	var result = {
		"death_revenge_activated": false,
		"revenge_mhp_activated": false
	}
	
	# on_death効果があるかチェック（早期リターン用）
	var items = defeated.creature_data.get("items", [])
	var has_on_death_effect = false
	for item in items:
		var effects = item.get("effect_parsed", {}).get("effects", [])
		for effect in effects:
			if effect.get("trigger", "") == "on_death":
				has_on_death_effect = true
				break
		if has_on_death_effect:
			break
	
	if not has_on_death_effect:
		return result
	
	# 各効果をチェック
	for item in items:
		var effects = item.get("effect_parsed", {}).get("effects", [])
		for effect in effects:
			if effect.get("trigger", "") != "on_death":
				continue
			
			match effect.get("effect_type", ""):
				"instant_death":  # 道連れ
					if _apply_instant_death_on_death(effect, opponent):
						result["death_revenge_activated"] = true
				
				"revenge_mhp_damage":  # 雪辱
					if opponent.is_alive():
						_apply_revenge_mhp_damage(effect, opponent, item)
						result["revenge_mhp_activated"] = true
	
	return result
```

### battle_participant.gd

```gdscript
## MHP範囲に直接ダメージ（雪辱効果用）
func take_mhp_damage(damage: int) -> void:
	"""
	ボーナスを無視してMHP（base_hp + base_up_hp）を直接削る
	ただし base_up_hp は永続ボーナスなため削られない
	MHPが0以下になった場合は即死扱い
	
	重要: base_up_hp は永続的なMHP増加であり、削られません
	"""
	# base_hp から消費（base_up_hp は削られない）
	if damage > 0:
		base_hp -= damage
	
	# 現在HPを再計算
	update_current_hp()
	
	# MHPが0以下になった場合は即死
	var current_mhp = base_hp + base_up_hp
	if current_mhp <= 0:
		base_hp = 0
		update_current_hp()
```

---

## 使用例

### 道連れの例

```gdscript
# バーニングハート装備のクリーチャーが撃破された
var death_effects = special_effects.check_on_death_effects(defeated, opponent)
if death_effects["death_revenge_activated"]:
	print("道連れ発動！相手も撃破")
```

**ログ出力例**:
```
【死亡時効果チェック】アモン
【道連れ発動】アモン → スチームギア (100% 判定成功)
  → スチームギア 道連れで撃破！
```

### 雪辱の例

```gdscript
# ナパームアロー装備のクリーチャーが撃破された
var death_effects = special_effects.check_on_death_effects(defeated, opponent)
if death_effects["revenge_mhp_activated"]:
	print("雪辱発動！相手のMHPを削った")
```

**ログ出力例**:
```
【死亡時効果チェック】アモン
【雪辱発動】アモンのナパームアロー → スチームギア
【MHPダメージ】スチームギア MHPに-40
  base_hp: -40 (残り:-10)
  → MHP=-10 即死発動
  → スチームギア 撃破！
```

---

## 設計思想

### なぜbattle_special_effects.gdに配置？

1. **特殊効果の集約**
   - 即死、無効化、再生など特殊な戦闘効果を1箇所に集約
   - battle_execution.gdは戦闘フローに集中

2. **拡張性**
   - 新しい死亡時効果を追加しやすい
   - match文に新しいeffect_typeを追加するだけ

3. **責任の分離**
   - BattleParticipant: ステータス管理
   - BattleExecution: 戦闘フロー
   - BattleSpecialEffects: 特殊効果処理

### triggerの統一

すべての死亡時効果で `"on_death"` を使用：
- 一貫性のある設計
- 検索・フィルタリングが容易
- 新しい効果の追加が簡単

### 双方向対応

侵略側/防衛側に関係なく動作：
```gdscript
check_on_death_effects(attacker, defender)  # 攻撃側が死亡
check_on_death_effects(defender, attacker)  # 防御側が死亡
```

### 複数効果の対応

1つのアイテムに複数の死亡時効果を持たせることも可能：
```json
{
  "effects": [
	{"effect_type": "instant_death", "trigger": "on_death", ...},
	{"effect_type": "revenge_mhp_damage", "trigger": "on_death", ...}
  ]
}
```

---

## 今後の拡張

### 追加可能な効果タイプ

```json
// HP回復
{
  "effect_type": "heal_opponent_on_death",
  "trigger": "on_death",
  "heal_amount": 30
}

// 魔力獲得（遺産）
{
  "effect_type": "grant_magic_on_death",
  "trigger": "on_death",
  "magic_amount": 50
}

// バフ付与
{
  "effect_type": "buff_opponent_on_death",
  "trigger": "on_death",
  "stat": "ap",
  "value": 20
}
```

すべて同じフレームワークで実装可能。

---

**最終更新**: 2025年11月2日（v1.0）
