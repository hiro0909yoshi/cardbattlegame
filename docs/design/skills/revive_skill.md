...
| クリーチャー | 411 | グレートフォシル | 強制 | ドラゴンゾンビ | 425 | なし |
| アイテム | 1045 | ネクロマンサーリング | 強制 | スケルトン | 420 | なし |

**注**: アイテムの蘇生は、アイテム使用者が撃破されたときに発動

---

## データ構造

蘇生効果は`ability_parsed.effects`配列に以下の形式で記述：
...
主要な処理:
1. `check_and_apply_revive()` - 蘇生チェックと適用
...
**問題**: 蘇生した後に両者生存で戦闘終了すると、復活**前**のクリーチャーが手札に戻る
...
**問題**: 蘇生後にクリーチャー交換しようとすると、`tile_info.has_creature`が`false`になり交換が失敗
...
**問題**: 蘇生後も戦闘が継続し、復活したクリーチャーが攻撃してしまう
...
for i in range(attack_order.size()):
	...
	if revive_result["revived"]:
		print("  【蘇生成功】", revive_result["new_creature_name"], "として復活！")
