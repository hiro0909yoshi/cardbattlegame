...
特殊移動は、通常の隣接移動を超えた移動能力の総称です。瞬移、敵地移動など、様々なバリエーションが存在します。
...
| タイミング | 種類 | 例 | 実装優先度 |
|-----------|------|-----|----------|
| **ドミニオオーダー** | 瞬移、敵地移動 | ブリーズスピリット、サンダースポーン | **高（Phase 1）** |
...
|----|-----|------|----------|-----------|
| 337 | ブリーズスピリット | 風 | 瞬移[風水] | 風・水属性の空き地 |
| 348 | ワイバーン | 風 | 先制；瞬移[風] | 風属性の空き地 |
| 229 | ドリアード | 地 | 加勢[無火地]；瞬移[地] | 地属性の空き地 |

### 敵地移動クリーチャー

| ID | 名前 | 属性 | スキル詳細 | 移動可能先 |
|----|-----|------|----------|-----------|
| 318 | サンダースポーン | 風 | 強襲[属性の違う敵ドミニオ] | 隣接タイル全て + 今いるタイルと異なる属性の敵地 |
...
| 2045 | エアリアル | 20MP | ドミニオ効果"天駆"付与 | ドミニオ効果 |
...
func _execute_move_action() -> void:
	...
	if _has_vacant_move_skill(creature):
		_show_message("瞬移: 対象属性の空き地を選択してください")
...
既存の移動処理（`execute_move`）をそのまま使用可能。瞬移も通常移動も、移動実行自体は同じ処理：
...
4. **ダウン状態の土地からは枷** - 通常移動と同じ

### バランス考慮
- 瞬移は強力なので、属性制限でバランスを取る
- 全属性移動可能な「瞬移[全]」は慎重に扱う
