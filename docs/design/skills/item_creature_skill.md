...
2. **アイテムとして使用** - 戦闘時に援護アイテムとして使用

### アイテムとして使用時の効果
- 基礎AP/HPがボーナスとして加算される
- 追加スキル（道連れ、死者復活等）も適用される
...
| 439 | リビングアムル | 20 | 10 | 敵武器不使用時、死者復活[リビングアムル] |
| 440 | リビングクローブ | 0 | 30 | AP&HP=他属性の配置数×5 |
| 441 | リビングヘルム | 20 | 30 | なし（基本効果のみ） |
| 442 | リビングボム | 10 | 30 | 道連れ[HP20以下] |
...
- [x] リビングアムル: 死者復活[リビングアムル]（敵武器不使用時）
- [x] リビングボム: 道連れ[HP20以下]（自分のHP20以下で自爆＋道連れ）
...
```
カード選択UI（アイテム選択時）
  ↓ type == "creature" でも「アイテムクリーチャー」キーワードがあれば選択可能
  ↓
battle_item_applier.gd
  ↓ アイテムクリーチャー判定（type == "creature"）
  ↓ 基礎AP/HPをitem_bonus_ap/hpとして加算
  ↓ ability_parsed.effectsを戦闘クリーチャーにマージ（スキル継承）
  ↓ keywordsもマージ（道連れ等のキーワードスキル用）
```
...
アイテムとして使用時、アイテムクリーチャーのスキルは戦闘クリーチャーに継承される。
...
アイテム: リビングボム（AP10/HP30、道連れ[HP20以下]）

結果:
- ファイターのAP: 30 + 10 = 40
- ファイターのHP: 40 + 30 = 70
- ファイターに「道連れ[HP20以下]」スキル付与
```
...
```json
{
  ...
  "ap": 0,
  "hp": 30,
  "ability_detail": "アイテムクリーチャー；AP&HP=他属性の配置数×5",
  "ability_parsed": {
	"keywords": ["アイテムクリーチャー"],
	"effects": [
	  {
