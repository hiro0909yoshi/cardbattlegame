...
**実装範囲**:
- `_check_nullify()` 関数
- `_check_nullify_element()` 関数
- `_check_nullify_mhp_above()` 関数
- ダメージ処理への統合

### Phase 2: データ追加 + 実装

3. 🔶 **複数属性無効化** - アーマーナイト、アクアドラゴン
4. 🔶 **MHP30以下無効化** - アクアリング
5. 🔶 **ST条件無効化** - シーホース
6. 🔶 **全攻撃軽減** - ガスクラウド
7. 🔶 **能力持ち無効化** - グレートタスカー
...
**実装範囲**:
- ability_parsedデータ追加
- `_check_nullify_mhp_below()` 関数
- `_check_nullify_st_below()` 関数
- `_check_nullify_has_ability()` 関数
- 軽減率処理の実装

### Phase 3: 巻物実装後

8. ❌ **巻物攻撃無効化** - アームドパラディン、イド
9. ❌ **通常攻撃無効化（条件付き）** - アブサス

**前提**:
- 巻物攻撃スキルの実装
- `is_using_scroll`フラグの実装
