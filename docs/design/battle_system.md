...
バージョン: 1.3
最終更新: 2025年12月16日（v1.3 - 移動侵略・特殊処理追記、ファイルパス修正）
...
移動侵略
ドミニオオーダーの「移動」で敵地に移動した場合、移動侵略となる。
通常の侵略との違い
項目通常侵略移動侵略攻撃側クリーチャー手札から出す配置済みクリーチャー土地ボーナス攻撃側になし攻撃側になし（移動元から離れるため）敗北時カード破壊移動元に戻る（HPダメージ維持）勝利時土地獲得＋配置土地獲得＋配置、移動元は空き地に相打ち時カード破壊両方破壊、移動元も空き地に
実装
```gdscript
# from_tile_index >= 0 の場合は移動侵略
func _execute_battle_core(..., from_tile_index: int = -1):
	# バトル実行...
	
	# 結果処理で from_tile_index を参照
	match result:
...
変身処理（バルダンダース等）
```
1. 戦闘開始時変身 (on_battle_start)
   - BattlePreparation内で TransformSkill.process_transform() を呼ぶ
   - 一時変身の場合、元データを battle_result["attacker_original"] 等に保存

2. 戦闘終了時復帰
   - _apply_post_battle_effects() 内で元に戻す
   - TransformSkill.revert_transform() を呼ぶ
