...
| 配置制限 / アイテム制限 | `restrictions.cannot_summon`, `restrictions.cannot_use` | 常時（横並び） |
| 呪い | `curse_effects` | 常時 |
...
| 8 | アイテム制限 | `restrictions.cannot_use` | 配置制限の右隣に表示 | 常時 |
...
**アイテム制限の表示例：**
- `restrictions.cannot_use: ["武器"]` → `アイテム制限: 武器`
- `restrictions.cannot_use: ["武器", "防具"]` → `アイテム制限: 武器,防具`
- `restrictions.cannot_use: null` → `アイテム制限: なし`
