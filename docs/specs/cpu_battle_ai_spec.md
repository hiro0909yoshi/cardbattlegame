...
2. **即死スキル対応** ✅ 完全実装
   ...
   **防御側判断フロー:**
   ...
   1. 無効化スキルで勝てる → アイテム温存
   2. 合体で勝てる → 合体
   3. 敵が100%即死スキル持ち＆自分に適用される場合:
	  → 通常攻撃100%無効化アイテムを探す
	  → 見つかれば使用
	  → 見つからなければアイテムを使わない（即死されるため無駄）
   4. 敵が確率即死（100%未満）の場合 → 通常判断へ
   5. 通常のシミュレーション判断（防具/援護）
   ```
   
   **即死対策の無効化アイテム条件:**
   - `nullify_type: "normal_attack"` かつ `reduction_rate` なし（または0.0）
   - 巻物攻撃無効化（`scroll_attack`）は即死対策にならない
   - ダメージ軽減（`reduction_rate: 0.2`等）は即死を防げない
...
| 2026-01-08 | 防御側: 100%即死対策の無効化アイテム判断を厳密化（通常攻撃100%無効化のみ、無効化なしならアイテム不使用） |
