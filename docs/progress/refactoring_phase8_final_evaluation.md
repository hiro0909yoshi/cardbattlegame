...
**結論**: 形式的な指標では依然として神オブジェクト。ただし**実質的な責務は 4 サービスに分離済み**。UIManager は「facade + コンポーネントライフサイクル管理」に収束しつつある。
...
**Phase 8 は「依存方向の固定」という主目標を達成した。**

- ビジネスロジック層（ハンドラー）→ UI層: **Signal 駆動**（37 Signals）
- 外部ファイル → UI操作: **サービス直接注入**（17ファイルで実施済み）
- UIManager: **facade + コンポーネントライフサイクル管理** に収束

UIManager の行数（1,024行）は計画の200行に達していないが、これは段階的移行の中間状態として妥当。委譲メソッドの一斉削除は将来的に可能だが、現時点ではリグレッションリスクとメリットのバランスから延期が正しい判断。
...
| # | 内容 | 効果 | リスク | 推奨時期 |
|---|------|------|--------|---------|
| 1 | `restore_current_phase()` 解体 | 後方参照 2件削除（SPH, DCH）、44行削減 | 中（カード閲覧モードの設計変更要） | Phase 9 |
| 2 | `update_player_info_panels()` サービス化 | 残存参照の最大要因を解消 | 低 | Phase 9 |
| 3 | card.gd 再帰探索の解消 | 32参照削除、脆い実装の改善 | 中（card.gd 改修） | Phase 10 |
| 4 | 委譲メソッド一斉削除 | UIManager ~400行削減 | 高（47ファイル変更） | Phase 10+ |
| 5 | tutorial 系移行 | 44参照削除 | 高（チュートリアル再設計） | 別プロジェクト |
