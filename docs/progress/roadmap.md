...
**共通**:
- [x] 利用規約の作成 → `docs/legal/terms_of_service.md`
- [x] プライバシーポリシーの作成（KIYAC生成） → `docs/legal/privacy_policy.md`
- [x] 特定商取引法に基づく表記 → `docs/legal/tokushoho.md`
- [ ] 掲載用 Web ページにホスティング（GitHub Pages 等、無料）

**法務関連（リリース前に必須）**:
- [ ] 利用規約・プライバシーポリシーの弁護士チェック（ガチャ・課金周り重点）
  - ゲーム特化の法律事務所推奨（弁護士法人LEON等）
- [ ] バーチャルオフィス契約 → 特商法表記の住所・電話番号を記入
  - 候補: GMOオフィスサポート（月660円〜）
- [ ] 各法務文書の「※リリース前に記入」箇所を埋める（氏名・住所・制定日）
- [ ] 法務文書をWebページとして公開（GitHub Pages等）→ ストア申請時にURL提出
- [ ] アプリ内から利用規約・プライバシーポリシー・特商法表記へのリンク設置
